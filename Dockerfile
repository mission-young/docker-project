...
RUN  echo "docker for scientific data analysis " \
    ...
    && apt-get install -y --fix-missing axel \
    ##### required minimum dependences for cern-root
    && rootMinDeps='git dpkg cmake g++ gcc binutils libx11-dev libxpm-dev libxft-dev libxext-dev' \
    && apt-get install -y  ${rootMinDeps} --fix-missing -o Acquire::http::proxy=false \ 
    ##### clear apt cache file
    && apt-get purge -y --auto-remove ${rootMinDeps} \
    ...
    # && apt-get purge -y --auto-remove $rootOptDeps \
